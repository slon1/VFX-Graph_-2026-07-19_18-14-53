...
На `Boids_mk1` alignment/cohesion/separation переведены на **AddNormalized*** + **HeadingSteer** ([ADR-012](../last/ADR-012-Kinematic-Heading-Boids.md)). `SteerToVelocityFieldPass` и `SampleGradientFieldPass` остаются в фреймворке для Gray-Scott-Boids / Newton-пресетов.
