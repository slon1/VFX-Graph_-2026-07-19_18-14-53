...
- (−) `ParticleRenderMode` в инспекторе временно ничего не делает — источник потенциальной путаницы, пока не решится вопрос GLES3. Подтверждено кодом: `SetupBinders` поле не читает.

**Уточнение 2026-09-25 (Play, не цвета LUT).** На коммите до этой ADR (рендер переключается, палитры нет) ленты Primitive при `particleSize` 0.05 реже и мигают относительно VFX Point на тех же пассах `Boids_mk1`. Число инстансов совпало с числом точек. Размер 0.2 почти убрал мигание: квад в мировых единицах уходит ниже пикселя, VFX Point держит минимум пиксель. Это не возвращает opt-in. Цвета по курсу (DoD Visual) по-прежнему не закрыты.
