...
Инспектор World правит **тот же** `EffectAsset` на диске: это ScriptableObject, не зависит от Ctrl+S и от сохранения сцены. Калибровка (`radiusUV`, `ε_vc`, Jacobi) всегда «в ассете». Бросовый look без порчи production — только копия ассета или будущий runtime-инстанс Assign, не «не нажимать Save».
